...
{
	std::vector<std::string> files = App->file_system->GetFilesInPath(App->file_system->GetLibraryTexturePath().c_str(), "dds");
...
{
	std::string path = App->file_system->GetPathFromFilePath(filepath);
	std::string filename = App->file_system->GetFileNameFromFilePath(filepath);
	std::string extension = App->file_system->GetFileExtension(filename.c_str());
	std::string name = App->file_system->GetFilenameWithoutExtension(filename.c_str(), false);

	std::string meta_path = path + filename + ".meta";
...
void ResourceTextureLoader::Import(const char * filepath)
{
	...
	std::string name = App->fs->GetFilenameWithoutExtension(filename.c_str());
...
bool ResourceTextureLoader::Export(const char * path, ResourceTexture* resource)
{
	...
	std::string name = resource->GetUniqueId();
	...
	if (size > 0)
	{
		...
		if (ilSaveL(IL_DDS, data, size) > 0)
			ret = App->fs->FileSave(path, (char*)data, name.c_str(), "dds", size);
